...
Я — твой персональный помощник по отслеживанию гарантийных сроков.

*📌 Что я умею:*
• Добавлять товары с датой окончания гарантии
• Напоминать за 30, 14, 7, 1 день и в день окончания
• Показывать все ваши товары
...
async def add_product_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "*📅 Введите дату окончания гарантии в формате ДД.ММ.ГГГГ или ДД.ММ.ГГ:*\n\n*Например:* 25.12.2024 *или* 25.12.24",
        reply_markup=cancel_menu(),
        parse_mode='Markdown'
    )
    return ADD_DATE
...
async def add_product_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        f"*Я напомню вам об окончании гарантии заранее!*",
...
async def edit_choice_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "edit_name":
        await query.edit_message_text(
            "*✏️ Введите новое название товара:*\n\n*Или нажмите* \"↩️ Отмена\" *для отмены*",
            parse_mode='Markdown'
        )
        return EDIT_NAME
        ...
        await query.edit_message_text(
            "*📅 Введите новую дату окончания гарантии (ДД.ММ.ГГГГ или ДД.ММ.ГГ):*\n\n*Или нажмите* \"↩️ Отмена\" *для отмены*",
            parse_mode='Markdown'
        )
        return EDIT_DATE
